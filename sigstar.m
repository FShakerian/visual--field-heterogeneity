function varargout=sigstar(groups,stats,nosort,C)
hold on
    ...
    %          assumed to be a vector of 0.1s the same length as groups. Nans are treated
    ...
    % 2. 
    % bar([5,2,1.5])
    % sigstar({[2,3],[1,2], [1,3]},[nan,0.1,0.1])
    %
    ...
    if nargin<2 
        stats=repmat(0.1,1,length(groups));
    end
    if isempty(stats)
        stats=repmat(0.1,1,length(groups));
    end
    ...
    for ii=1:length(groups)
        ...
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),C);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,C)
    ...
    H(1)=plot(x(:),y,'Color',C,'LineWidth',1.5,'Tag','sigstar_bar');
...
H(2)=text(mean(x(:)),starY,stars,...
        'HorizontalAlignment','Center',...
        'BackGroundColor','none',...
        'Tag','sigstar_stars','Color',C,'FontSize',14);
